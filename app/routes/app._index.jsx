...
const sendWhatsAppMessage = async () => {
  ...
  return (
    <div style={{ fontFamily: "Arial, sans-serif", maxWidth: 1200, margin: "auto" }}>
        ...
        <div style={{ display: "flex", height: 650, border: "1px solid #ccc", borderRadius: 8, overflow: "hidden" }}>
          ...
          <div style={{ width: "22%", borderRight: "1px solid #eee", overflowY: "auto" }}>
            ...
            {[...fbPages, ...igPages].map((page) => (
              <div key={page.id} onClick={() => fetchConversations(page)} style={{ padding: 12, cursor: "pointer", backgroundColor: selectedPage?.id === page.id ? "#e3f2fd" : "white", borderBottom: "1px solid #eee" }}>
            ...
            {shopifySessions.length > 0 && (
              <>
                <div style={{ padding: 12, borderBottom: "1px solid #ddd", background: "#f7f7f7", fontWeight: "600", marginTop: 10 }}>Shopify</div>
                {shopifySessions.map((s) => (
                  <div key={s.sessionId} onClick={() => openShopifyConversation(s)} style={{ padding: 12, cursor: "pointer", backgroundColor: selectedPage?.id === s.sessionId ? "#e3f2fd" : "white", borderBottom: "1px solid #eee" }}>
          ...
          <div style={{ width: "28%", borderRight: "1px solid #eee", overflowY: "auto" }}>
            ...
            {conversations.map((conv) => {
              let name;
              if (selectedPage?.type === "shopify") name = conv.userName || conv.sessionId || "Shopify User";
              else if (selectedPage?.type === "instagram") name = `${conv.businessName} ↔️ ${conv.userName}`;
              else if (selectedPage?.type === "whatsapp") name = conv.userName || conv.profile?.name || "WhatsApp User";
              else name = conv.participants?.data?.map((p) => p.name).join(", ") || "User";

              return (
                <div key={conv.id || conv.sessionId} onClick={() => selectedPage?.type === "shopify" ? openShopifyConversation(conv) : fetchMessages(conv)} style={{ padding: 12, cursor: "pointer", backgroundColor: selectedConversation?.id === conv.id ? "#e7f1ff" : "white", borderBottom: "1px solid #eee" }}>
                  {name}
                </div>
              );
            })}
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ flex: 1, padding: 12, overflowY: "auto", background: "#f9f9f9", display: "flex", flexDirection: "column" }}>
              {(messages[selectedConversation?.id || selectedConversation?.sessionId] || []).map((msg) => {
                const isMe = selectedPage?.type === "shopify" ? msg.displayName === "You" : msg.from === "me";
                return (
                  <div key={msg.id} style={{ display: "flex", flexDirection: "column", marginBottom: 8 }}>
                    <div style={{ alignSelf: isMe ? "flex-end" : "flex-start", backgroundColor: isMe ? "#d1e7dd" : "#f0f0f0", color: "#333", padding: "10px 15px", borderRadius: 15, maxWidth: "70%", wordBreak: "break-word", boxShadow: "0 1px 2px rgba(0,0,0,0.1)" }}>
            ...
            <div style={{ display: "flex", padding: 12, borderTop: "1px solid #ddd" }}>
              <input type="text" value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder="Type a message" style={{ flex: 1, padding: 10, borderRadius: 5, border: "1px solid #ccc" }} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); selectedPage?.type === "shopify" ? sendShopifyMessage() : sendMessage(); } }} />
              <button onClick={() => (selectedPage?.type === "shopify" ? sendShopifyMessage() : sendMessage())} style={{ marginLeft: 10, padding: "10px 20px", backgroundColor: "#007bff", color: "white", border: "none", borderRadius: 5, cursor: "pointer" }}>Send</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <style>{`
        .btn-primary { background-color: #000; color: white; padding: 10px 20px; border: none; border-radius: 4px; font-size: 16px; font-weight: 500; cursor: pointer; transition: background-color 0.3s ease; }
        .btn-primary:disabled { background-color: #555; cursor: not-allowed; }
        .btn-primary:not(:disabled):hover { background-color: #222; }
      `}</style>
    </div>
  );
}
