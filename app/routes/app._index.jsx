    ...
    >
      ...
      {!conversations.length ? (
        ...
        conversations.map((conv) => {
          const names = conv.participants?.data
            ?.filter(
              (p) =>
          ...
          // If no participant name, fallback to saved messages with name
          const fallbackName =
            !names?.length &&
            messages[conv.id]?.length &&
            messages[conv.id][0]?.name
              ? messages[conv.id][0].name
              : "Unknown";
          ...
          return (
            ...
            >
              {names?.length ? names.join(", ") : fallbackName}
            </div>
          );
        })
      )}
      ...
      >
        ...
        {activeConversation
          ? activeConversation.participants?.data
              ...
              .map((p) => p.name || p.username)
              .join(", ") ||
            // fallback to first message name
            (messages[activeConversation.id]?.[0]?.name || "Unknown")
          : ""}
      </h3>
      ...
      >
        ...
        messages[activeConversation.id].length ? (
          messages[activeConversation.id].map((msg, idx) => {
            const isMe =
              msg.sender === "me" || msg._tempId; // optimistic sent messages
        ...
        <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (f) {
                sendMessage("", f, nameInputRef.current.value || "Guest");
                e.target.value = "";
              }
            }}
          />
          ...
          <input
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                ...
                if (v) {
                  sendMessage(v, null, nameInputRef.current.value || "Guest");
                  e.target.value = "";
                }
              }
            }}
          ...
          <button
            ...
            onClick={() => {
              ...
              if (input && input.value.trim()) {
                sendMessage(input.value.trim(), null, nameInputRef.current.value || "Guest");
                input.value = "";
              }
            }}
          >
